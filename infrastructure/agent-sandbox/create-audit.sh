...
SCOPE="${ARGS[*]:-}"
if [ -z "$SCOPE" ]; then
  echo "Usage: pnpm agentic:audit [--model <model>] \"<audit scope>\"" >&2
  exit 1
fi
...
echo "Auditing codebase for: $SCOPE"
docker run --rm --init --name "vb-audit-$(date +%s)" \
  ...
  -e SOLVE_MODEL="$MODEL" \
  "$IMAGE" \
  bash -c 'exec bash "$HOME/vigilant-broccoli/infrastructure/agent-sandbox/create-audit-runner.sh" "$1"' _ "$SCOPE"
