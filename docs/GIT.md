...
- The upptime app is scoped to Contents + Issues RW only — nothing else. Workflows mint a short-lived (~1h) installation token per run via `infrastructure/agent-sandbox/mint-github-app-token.sh`, from an App ID + private key stored in Vault. Never reuse the broader Terraform/agent-sandbox GitHub credentials for a push-to-main use case — a long-lived, broadly-scoped token sitting in a job for a third-party action to read is a real credential-exposure surface; a purpose-built, narrowly-scoped, auto-expiring one bounds the blast radius if it ever leaks.
