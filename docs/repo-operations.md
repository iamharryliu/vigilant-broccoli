...
- **CI access**: Vault at `https://vault.harryliu.dev` (cloudflared tunnel, Cloudflare Access service token) + GitHub OIDC JWT auth, via `.github/actions/vault-secrets`. That action authenticates to GCP via Workload Identity Federation and pulls the Cloudflare Access token from Secret Manager, so the only plain GitHub secrets are `GCP_WORKLOAD_IDENTITY_PROVIDER` / `GCP_SERVICE_ACCOUNT` (the WIF bootstrap) plus the optional `GH_PAT` fallback (see [CI.md](../CI.md)).
...
- **Per-service fly secrets**: wired by `projects/nx-workspace/scripts/secrets-mapping.config.ts` (env-less `flyAppBaseName`) + each service's `.env.example`, pushed by `deploy-flyio-secrets.ts <project> [staging|production]` (`deploy:secrets` / `deploy:secrets:production` targets), which composes `<env>-<base>` (mechanics in [fly-service-pattern.md](../projects/nx-workspace/docs/api/deployment/fly-service-pattern.md)). Both environments read the same Vault path — per-env secret values would need per-env vault paths.
