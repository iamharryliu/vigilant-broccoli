...
├── drive.harryliu.dev                    Seafile — OCI VM (A record, proxied + Cloudflare Access, owner-email only; shares the code-server VM's Ampere pool budget — code-server was shrunk from 2 OCPU/14GB/50GB boot to 1 OCPU/6GB/30GB boot to fund Seafile's 1 OCPU/8GB/20GB single-volume instance)
