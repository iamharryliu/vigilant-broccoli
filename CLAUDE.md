...
- Never declare a dependency as `"*"` (or a stale range) in a lib/app `package.json` for a package already pinned in the workspace root `package.json` — pnpm only re-resolves an importer when its own specifier changes, so a `"*"` copy can silently drift to a different version than root and cause a confusing type error instead of an obvious version-mismatch one. Mirror the root's exact range instead. For packages prone to drift, add an entry to `overrides` in `pnpm-workspace.yaml` (not `package.json` — pnpm 11.x no longer reads `overrides`/`pnpm.overrides` there) to force one resolved version workspace-wide.
