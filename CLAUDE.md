...
- For UI applications (adding an app, shared components, i18n, pages-index cards), read `projects/nx-workspace/docs/ui/ui-app-pattern.md` first; each UI deploy destination has a pattern doc in `projects/nx-workspace/docs/deployment/` (`vercel-deploy-pattern.md`, `cloudflare-pages-deploy-pattern.md`, `github-pages-deploy-pattern.md`).
...
- UI applications must implement i18n for all user-facing copy, with an English locale at minimum, via the shared `createI18n` from `@vigilant-broccoli/react-lib` — never hardcode copy in components or define a per-app i18n mechanism. Dictionary format, provider wiring, and what does not count as translatable copy: see `ui-app-pattern.md` above.
