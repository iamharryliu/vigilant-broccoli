...
1. Read `TODO.md` to learn its structure: the section headings (e.g. `## Features to Add`, `## P1`–`## P3`) and entry format — `<6-hex-id>. [category] Title`, a short context paragraph, then steps. Entry heading depth is one level past its immediate parent heading — a direct child of a `##` section is `###`, but a section with an intermediate subsection (e.g. `## Feature` > `### Enhancements`) pushes its entries to `####`. Match whatever depth the section you're adding to already uses; don't assume `###` everywhere.
...
5. Add the entry under the most fitting section (create the section if the user asked for a new one):
   - `### <id>. Title` (or `####` if the section nests a nameless subsection — see step 1) — include a `[security]`/`[performance]`/`[maintenance]` tag when it matches the section's style.
