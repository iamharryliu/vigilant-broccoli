...
2. Check `git status` and `git diff` (staged and unstaged). Stage only the files this session created or edited, per the staging rule; if unsure whether a file was touched this session, leave it unstaged and ask rather than guessing from the diff. Anything staged that this session did not stage belongs to a peer — leave it alone.
