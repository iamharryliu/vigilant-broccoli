...
- **vb-storage-service** (bucket-service): **done** — locked down to 6PN. Its only caller, `apps/ui/vb-manager-next/src/app/api/bucket/route.ts:9-27`, turned out to be local-only (pm2, no `deploy` target), not Vercel-hosted as this entry originally assumed, so no gateway was needed.
- **email-subscription-service** itself has public `/api/subscribe`, `/api/unsubscribe`, `/api/notify` routes (`apps/api/email-subscription-service/src/main.ts:167-278`) with no in-repo caller found — needs a decision on whether it's intentionally a public-facing API (future newsletter signup) or can be locked down too. If not, it is the next zero-caller lockdown after bucket-service.
