...
export default function ProjectsPage() {
  ...
  const fetchProjects = useCallback(async () => {
    ...
    const res = await fetch(`/api/projects?homeId=${homeId}`, {
      headers: authHeader(),
    });
  ...
  useEffect(() => {
    fetchProjects();
    fetchCalendarEvents();
  }, [fetchProjects, fetchCalendarEvents]);
  ...
  const handleEdit = async (id: string, data: HomeProjectFormData) => {
    await fetch('/api/projects', {
      method: 'PATCH',
      headers: authHeader({ 'Content-Type': 'application/json' }),
  ...
  const handleConfirmDrop = async (data: CalendarEventFormData) => {
    await fetch('/api/calendar/events', {
      ...
      body: JSON.stringify({
        ...data,
        homeId,
        projectId: pendingDrop?.projectId ?? null,
      }),
    });
    setPendingDrop(null);
    fetchCalendarEvents();
