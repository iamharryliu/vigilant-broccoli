...
export default function LeisureDetailPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      <Link href={ROUTES.LEISURE} className="text-sm text-gray-500 hover:text-gray-700">
        ← Back
      </Link>
      ...
      {activity.createdByEmail && (
        <Text size="2" color="gray" as="p">Added by {activity.createdByEmail}</Text>
      )}

      <Text size="1" color="gray" as="p">
        Added {new Date(activity.createdAt).toLocaleDateString(undefined, { dateStyle: 'medium' })}
      </Text>
    </div>
  );
}
